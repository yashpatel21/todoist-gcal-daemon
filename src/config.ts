...
/**
 * Loads and validates config. Throws ConfigError on bad or missing values.
 * Refresh token is optional so the OAuth bootstrap can run when it is absent.
 */
...
/**
 * Like optionalEnv, but keeps leading/trailing whitespace and allows "".
 * Useful when `"Todoist: "` and `""` are both valid (empty means no prefix).
 */
